...
public class PID implements Runnable {
	// ------------Variáveis do PID-----------------------------
	public static float PID = 0, // valor final do PID para calculo da
									// velocidade das rodas
			e = 0, // erro, diferença de angulo entre o valor de zero do gyro e
					// o valor lido do gyro
			...
			Kd = 0.5f; // parametro do controle derivativo
	public static float[] WdWe = new float[2]; // Velocidade angular da roda
												// Direita e Esquerda
	/**
	 * variavel que determina se a Thread do PID esta pausada ou rodando,
	 * lembre-se de reajustar a velocidade dos motores quando necessario.
	 */
	public static boolean pidRunning = false; // seta a pausa do PID mas nao é
												// instantaneo
	public static boolean PIDparado = false; // o PID ja esta parado?

	// ------------Metodos do PID-----------------------------
	/**
	 * metodo que zera: PID, erro, erro anterior, angulo esperado (objetivo) e
	 * angulo real (lido pelo gyro)
	 */
		...
		while (AlienRescue.alienRescueON) {
			...
			while (!pidRunning && AlienRescue.alienRescueON) {
				...
				PIDparado = true; // indica que o pid realmente esta parado e
									// que pode ser zerado sem preigo de ocorrer
									// mais uma iteracao que mude
			}
		}
	}
	...
	public static void setWdWePID() {
		WdWe[0] = (2 * (float) (180 / Math.PI) * Navigation.VELO_INI - Navigation.DISTANCIA_ENTRE_RODAS * PID)
				/ (2 * Navigation.RAIO);
		WdWe[1] = (2 * (float) (180 / Math.PI) * Navigation.VELO_INI + Navigation.DISTANCIA_ENTRE_RODAS * PID)
				/ (2 * Navigation.RAIO);
